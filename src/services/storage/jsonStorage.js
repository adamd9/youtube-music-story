...
/**
 * Save a new playlist to JSON file
 */
async function savePlaylist({ ownerId, title, topic, summary, timeline, source, narrationAlbumArtUrl, _debug }) {
  ...
  dbg('jsonStorage: saved playlist', { id, ownerId });
  return record;
}

/**
 * Get a playlist by ID from JSON file
 */
...
/**
 * List playlists by owner from JSON files
 */
...
/**
 * Update a playlist in JSON file
 */
async function updatePlaylist(id, partial) {
  ...
  await fsp.rename(tmpPath, filePath);
  dbg('jsonStorage: updated playlist', { id });
  return merged;
}

/**
 * List all playlists from JSON files (for migration)
 */
async function listAllPlaylists() {
  ...
  for (const f of files) {
    ...
    try {
      ...
      console.error(`jsonStorage: failed to read ${f}`, e.message);
    }
  }
  return results;
}
...
/**
 * Check if there are any JSON playlist files
 */
